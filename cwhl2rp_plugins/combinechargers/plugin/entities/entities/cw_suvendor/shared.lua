...
function ENT:SpawnFunction(client, trace)
	...
	for k, v in pairs(ents.FindInBox(entity:LocalToWorld(entity:OBBMins()), entity:LocalToWorld(entity:OBBMaxs()))) do
		if (string.find(v:GetClass(), "prop") and v:GetModel() == "models/props_combine/suit_charger001.mdl") then
    ...
    if (!self.nextUse or CurTime() >= self.nextUse) then
		if (Schema:PlayerIsCombine(activator)) then
			if (activator:Armor() < 100) then
				...
				activator:EmitSound("items/suitchargeok1.wav", 55, 100)
				activator:SetArmor(100);
			else
				self.nextUse = CurTime() + 2;	
				activator:EmitSound("items/suitchargeno1.wav", 55, 100)
			end;
		else
			self.nextUse = CurTime() + 2;
			activator:EmitSound("items/suitchargeno1.wav", 55, 100)
