...
function ITEM:OnChangedClothes(player, bIsWearing)
	if (bIsWearing) then
		...
		-- Find gender and use it to set a model
		if (player:GetGender() == GENDER_MALE) and (self("replacementmale") != nil) then
			self.replacement = table.Random(self.replacementmale)
		elseif (player:GetGender() == GENDER_FEMALE) and (self("replacementfemale") != nil) then
			self.replacement = table.Random(self.replacementfemale)
		end
	else
...
function ITEM:OnUse(player, itemEntity)
	if (player:Alive() and !player:IsRagdolled()) then
		if (!self.CanPlayerWear or self:CanPlayerWear(player, itemEntity) != false) then
			...
			-- Set their new faction and reload the character
			if self("faction") != nil then
				self.oldfaction = player:GetFaction()
				player:SetCharacterData("Faction", self("faction"), true)
...
function ITEM:OnPlayerUnequipped(player, extraData)
	...
	-- Return player to old faction and reload them
	if self("faction") != nil and self("oldfaction") != nil then
		player:SetCharacterData("Faction", self("oldfaction"), true)
		Clockwork.player:LoadCharacter(player, Clockwork.player:GetCharacterID(player));
	end
